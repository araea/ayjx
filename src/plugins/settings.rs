...
const SETTINGS: &[SettingSpec] = &[
    ...
    s("ai_news", "image_enabled", "资讯卡片图", "一级内容只发卡片图，引用图片后按需提取正文与链接", Kind::Bool, "true"),
    s("ai_news", "image_max_items", "卡片图条数", "卡片最多展示几条；其余内容可用“提取全部”获取", Kind::Int, "10"),
    s("ai_news", "card_theme", "资讯阅读主题", "auto 按北京时间自动切换，也可设 light 或 dark；即时生效", Kind::Str, "auto"),
    s("ai_news", "forward_threshold_chars", "合并转发阈值", "批量提取或纯文本兜底超过多少字时改用合并转发，0 为永远纯文本", Kind::Int, "500"),
    s("ai_news", "forward_node_chars", "转发节点长度", "批量提取使用合并转发时每个节点的字数软上限", Kind::Int, "300"),
