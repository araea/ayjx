...
const SETTINGS: &[SettingSpec] = &[
    ...
    s(
        ...
        "mode",
        "手动查询范围",
        "仅影响 /ai资讯：selected 查精选（推荐），all 查全部；主动推送策略固定",
        Kind::Str,
        "selected",
    ),
    s(
        ...
        "realtime_mode",
        "实时资讯范围",
        "selected 仅推精选（推荐、低干扰），all 推送全部资讯；也可用 /ai实时模式切换",
        Kind::Str,
        "selected",
    ),
    s(
    ...
    s(
        ...
        "category",
        "资讯分类过滤",
        "ai-models/ai-products/industry/paper/tip，留空为不限",
        Kind::Str,
        "",
    ),
    s(
    ...
    s(
        ...
        "card_theme",
        "资讯阅读主题",
        "auto 按北京时间自动切换，也可设 light 或 dark；即时生效",
        Kind::Str,
        "auto",
    ),
    s(
        "ai_news",
        "forward_threshold_chars",
        "合并转发阈值",
        "批量提取或纯文本兜底超过多少字时改用合并转发，0 为永远纯文本",
        Kind::Int,
        "500",
    ),
    s(
    ...
    s(
        ...
        "实时轮询间隔",
        "每隔多少秒查一次新资讯；默认采用上游允许的 60 秒下限",
        Kind::Int,
        "60",
    ),
    s(
        "ai_news",
        "realtime_max_age_minutes",
        "实时保鲜期",
        "只推收录时间在此分钟数内的条目；默认覆盖最近 24 小时",
        Kind::Int,
        "1440",
    ),
    s(
    ...
    s(
        "ai_news",
        "realtime_max_per_hour",
        "实时频次容量",
        "每个群每小时最多实时推送次数；默认 60，正常情况下等同不限制",
        Kind::Int,
        "60",
    ),
    s(
    ...
    s(
        ...
        "memory_threshold_mb",
        "内存重启阈值",
        "进程内存超过该值（MB）自动重启，0 关闭",
        Kind::Int,
        "0",
    ),
    s(
        "restart",
        "allow_manual_restart",
        "允许手动重启",
        "是否允许群聊发送 /restart 手动重启",
        Kind::Bool,
        "false",
    ),
    s(
        ...
        "restart_command",
        "外部重启命令",
        "如 systemctl restart xxx；留空则进程自我拉起",
        Kind::Str,
        "",
    ),
];
...
pub fn handle(
    ...
    Box::pin(async move {
        for trigger in TRIGGERS {
            ...
            let result = crate::plugins::ctl::set_value(&ctx, plugin, spec.key, new_value).await;
            let text = match result {
                Ok(text) => text,
                Err(e) => format!("❌ {e}"),
            };
            let reply = Message::new().text(text);
