...
export default function(pi: ExtensionAPI) {
  ...
  register("satori_group", "查这个群的现成资料：某人的群名片/头衔/入群时间/多久没冒头（member）、群人数与活跃概况（roster）、最活跃或最久没说话的人（activity）、快到入群周年的人（anniversary）、随机抽人（draw）、随机分队（teams）、群文件目录或某个文件的下载链接（files）。群友说「抽个人」「分下队」「这人谁啊」「群文件里那个」时正好用；也可以拿来看眼前这位是老熟人还是新面孔。全是只读查询，不改群设置，每轮有查询次数上限。", Type.Object({
  ...
  register("satori_memo", "把以后还想记得的事写进长期记忆：对某个群友的一句印象、群里刚起的梗。挑那种会改变你以后怎么对待这个人或这个话题的一句写，一句话就够。记岔了随时改写（同一个人再写一次即可）或删掉。不占发送额度，记了什么是你自己的事。", Type.Object({
