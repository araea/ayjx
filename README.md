...
默认 Satori 地址为 `http://127.0.0.1:3001`。网页截图与资讯长图需要本机 Chrome 或 Chromium，可在 `browser_path` 指定路径。浏览器不可用时，帮助与插件控制回退纯文本。中文出图需要系统中日韩字体，只有 Regular 一档时标题会用合成的伪粗体，跑 `sh scripts/install-cjk-weights.sh` 装上真字重即可。
...
任意房间都可以交给内置 agent：`##研究 pi` 建房，`助手%pi apilio/claude-opus-5` 换引擎与模型，房间名不受限制。它自己调模型、自己用工具，不需要额外安装任何东西。`[oai.ambient]` 可让同一个 agent 以固定人格在指定群里旁听、偶尔搭话。普通房间的模型可写 `供应商/模型`（供应商在 `[oai].providers` 里配，如 `deepseek/deepseek-flash`），并支持 `:强度` 后缀或房间的 `thinking` 字段指定思考强度。

内置 agent 还能联网：`web_search` 查最新信息、`web_fetch` 读网页正文，后端与密钥在 `[oai.search]` 里配一次，房间与搭话共用。房间默认关闭（`[oai.search].enabled`），搭话默认开启（`[oai.ambient].search_enabled`）；默认走免密钥的 Bing / DuckDuckGo 抓取，什么都不配就能用。
